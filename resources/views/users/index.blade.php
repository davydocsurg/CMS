                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="text-left">
                                        <img src="{{ asset($user->profile->avatar) }}" class="rounded-circle" height=""
                                            alt="" style=" border:.1rem solid white; width:3rem">
                                    </td>
                                    <td>{{ $user->name }}</td>
                                    <td>
                                        {{ $user->email }}
                                    ...
                                    <td>
                                        ...
                                        @if (Auth::id() == 1 && Auth::id() !== $user->id)
                                            @if (!$user->isAdmin())
                                                <div class="">
                                                    <form action="{{ route('users.make_admin', $user->id) }}"
                                                        method="post">
                                                        @csrf
                                            ...
                                            @else
                                                <div class="">
                                                    {{-- @if (!$user->isWriter()) --}}
                                                    <form action="{{ route('users.remove_admin', $user->id) }}"
                                                        method="post">
                                                        @csrf
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content bg-dark">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="deleteModalLabel">Delete User</h5><button
                                            class="close text-white" type="button" data-dismiss="modal"
                                    ...
                                    <div class="modal-footer"><button class="btn btn-secondary btn-sm" type="button"
                                            data-dismiss="modal">Cancel</button><button class="btn btn-danger btn-sm"
                                            type="submit">Yes, Delete</button></div>
                                </div>
                            </div>
                        </div>
                    </form>
